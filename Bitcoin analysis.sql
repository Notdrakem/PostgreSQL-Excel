...
--% increases month to month, highest ranked months in % increase.

SELECT 
	date,
	ROUND(CAST(bitcoin_avg AS NUMERIC), 2) AS bitcoin_avg,
	ROUND(CAST(COALESCE(bitcoin_avg - LAG(bitcoin_avg, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2) AS month_to_month_bitcoin,
	TO_CHAR(ROUND(CAST(ROUND(CAST(COALESCE(bitcoin_avg - LAG(bitcoin_avg, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2)/COALESCE(LAG(bitcoin_avg, 1) OVER (ORDER BY date), 1) * 100 AS NUMERIC), 2), '%99D99') AS percentage_diff_bitcoin_m2m,
	ROUND(CAST(avg_sp500_month AS NUMERIC), 2) AS avg_sp500_month,
	ROUND(CAST(COALESCE(avg_sp500_month - LAG(avg_sp500_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2) AS month_to_month_sp500,
	TO_CHAR(ROUND(CAST(ROUND(CAST(COALESCE(avg_sp500_month - LAG(avg_sp500_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2)/COALESCE(LAG(avg_sp500_month, 1) OVER (ORDER BY date), 1) * 100 AS NUMERIC), 2), '%99D99') AS percentage_diff_sp500_m2m,
	ROUND(CAST(avg_corn_month AS NUMERIC), 2) AS avg_corn_month,
	ROUND(CAST(COALESCE(avg_corn_month - LAG(avg_corn_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2) AS month_to_month_corn,
	TO_CHAR(ROUND(CAST(ROUND(CAST(COALESCE(avg_corn_month - LAG(avg_corn_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2)/COALESCE(LAG(avg_corn_month, 1) OVER (ORDER BY date), 1) * 100 AS NUMERIC), 2), '%99D99') AS percentage_diff_corn_m2m,
	ROUND(CAST(avg_soybean_month AS NUMERIC), 2) AS avg_soybean_month,
	ROUND(CAST(COALESCE(avg_soybean_month - LAG(avg_soybean_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2) AS month_to_month_soy,
	TO_CHAR(ROUND(CAST(ROUND(CAST(COALESCE(avg_soybean_month - LAG(avg_soybean_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2)/COALESCE(LAG(avg_soybean_month, 1) OVER (ORDER BY date), 1) * 100 AS NUMERIC), 2), '%99D99') AS percentage_diff_soy_m2m,
	ROUND(CAST(avg_wheat_month AS NUMERIC), 2) AS avg_wheat_month,
	ROUND(CAST(COALESCE(avg_wheat_month - LAG(avg_wheat_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2) AS month_to_month_wheat,
	TO_CHAR(ROUND(CAST(ROUND(CAST(COALESCE(avg_wheat_month - LAG(avg_wheat_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2)/COALESCE(LAG(avg_wheat_month, 1) OVER (ORDER BY date), 1) * 100 AS NUMERIC), 2), '%99D99') AS percentage_diff_wheat_m2m,
	ROUND(CAST(avg_oil_month AS NUMERIC), 2) AS avg_oil_month,
	ROUND(CAST(COALESCE(avg_oil_month - LAG(avg_oil_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2) AS month_to_month_oil,
	TO_CHAR(ROUND(CAST(ROUND(CAST(COALESCE(avg_oil_month - LAG(avg_oil_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2)/COALESCE(LAG(avg_oil_month, 1) OVER (ORDER BY date), 1) * 100 AS NUMERIC), 2), '%99D99') AS percentage_diff_oil_m2m
FROM combined_data_vis;
...
(SELECT 
	...
	ROUND(CAST(bitcoin_avg AS NUMERIC), 2) AS bitcoin_avg,
	ROUND(CAST(COALESCE(bitcoin_avg - LAG(bitcoin_avg, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2) AS month_to_month_bitcoin,
	ROUND(CAST(ROUND(CAST(COALESCE(bitcoin_avg - LAG(bitcoin_avg, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2)/COALESCE(LAG(bitcoin_avg, 1) OVER (ORDER BY date), 1) * 100 AS NUMERIC), 2) AS percentage_diff_bitcoin_m2m,
	ROUND(CAST(avg_sp500_month AS NUMERIC), 2) AS avg_sp500_month,
	ROUND(CAST(COALESCE(avg_sp500_month - LAG(avg_sp500_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2) AS month_to_month_sp500,
	ROUND(CAST(ROUND(CAST(COALESCE(avg_sp500_month - LAG(avg_sp500_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2)/COALESCE(LAG(avg_sp500_month, 1) OVER (ORDER BY date), 1) * 100 AS NUMERIC), 2) AS percentage_diff_sp500_m2m,
	ROUND(CAST(avg_corn_month AS NUMERIC), 2) AS avg_corn_month,
	ROUND(CAST(COALESCE(avg_corn_month - LAG(avg_corn_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2) AS month_to_month_corn,
	ROUND(CAST(ROUND(CAST(COALESCE(avg_corn_month - LAG(avg_corn_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2)/COALESCE(LAG(avg_corn_month, 1) OVER (ORDER BY date), 1) * 100 AS NUMERIC), 2) AS percentage_diff_corn_m2m,
	ROUND(CAST(avg_soybean_month AS NUMERIC), 2) AS avg_soybean_month,
	ROUND(CAST(COALESCE(avg_soybean_month - LAG(avg_soybean_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2) AS month_to_month_soy,
	ROUND(CAST(ROUND(CAST(COALESCE(avg_soybean_month - LAG(avg_soybean_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2)/COALESCE(LAG(avg_soybean_month, 1) OVER (ORDER BY date), 1) * 100 AS NUMERIC), 2) AS percentage_diff_soy_m2m,
	ROUND(CAST(avg_wheat_month AS NUMERIC), 2) AS avg_wheat_month,
	ROUND(CAST(COALESCE(avg_wheat_month - LAG(avg_wheat_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2) AS month_to_month_wheat,
	ROUND(CAST(ROUND(CAST(COALESCE(avg_wheat_month - LAG(avg_wheat_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2)/COALESCE(LAG(avg_wheat_month, 1) OVER (ORDER BY date), 1) * 100 AS NUMERIC), 2) AS percentage_diff_wheat_m2m,
	ROUND(CAST(avg_oil_month AS NUMERIC), 2) AS avg_oil_month,
	ROUND(CAST(COALESCE(avg_oil_month - LAG(avg_oil_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2) AS month_to_month_oil,
	ROUND(CAST(ROUND(CAST(COALESCE(avg_oil_month - LAG(avg_oil_month, 1) OVER (ORDER BY date), 0) AS NUMERIC), 2)/COALESCE(LAG(avg_oil_month, 1) OVER (ORDER BY date), 1) * 100 AS NUMERIC), 2) AS percentage_diff_oil_m2m
FROM combined_data_vis)

SELECT
	date,
	percentage_diff_bitcoin_m2m,
	RANK () OVER (ORDER BY percentage_diff_bitcoin_m2m DESC) AS rank_btc,
	month_to_month_sp500,
	RANK () OVER (ORDER BY month_to_month_sp500 DESC) AS rank_sp,
	month_to_month_corn,
	RANK () OVER (ORDER BY month_to_month_corn DESC) AS rank_corn,
	month_to_month_soy,
	RANK () OVER (ORDER BY month_to_month_soy DESC) AS rank_soy,
	month_to_month_wheat,
	RANK () OVER (ORDER BY month_to_month_wheat DESC) AS rank_wheat,
	month_to_month_oil,
	RANK () OVER (ORDER BY month_to_month_oil DESC) AS rank_oil
FROM _changes
ORDER BY rank_corn
LIMIT 10;

--top 10 months can order by each field, not a clear pattern among them, however, it would appear wheat and soy have the closest relationship